...
// 2D LUT grading
// scaleOffset = (1 / lut_width, 1 / lut_height, lut_height - 1)  (1 / 32*32, 1 / 32, 31)
// uvw Logc空间的RGB
real3 ApplyLut2D(TEXTURE2D_PARAM(tex, samplerTex), float3 uvw, float3 scaleOffset)
{
    // Strip format where `height = sqrt(width)`
    uvw.z *= scaleOffset.z;// b * 31 
    float shift = floor(uvw.z);//向下取整 Index
    // y = g * 31 / 32 + 1 / 32 * 0.5
    // x = r * 31 * 1 / (32 * 32) + (1 / (32 * 32) * 1 / 2)
    // x += floor(b * 31) * 1 / 32
...
// params = (lut_height, 0.5 / lut_width, 0.5 / lut_height, lut_height / lut_height - 1) lut_width = 32 
// lut_height为32时，在一格内约等于r(0-1), g(0-1), b(0-1/32)
// 将uv转换为一个rgb值 
//r = frac(uv.x * 32); g = uv.y; b = uv.x - r / 32
//uv.y = g uv.x = 
real3 GetLutStripValue(float2 uv, float4 params)
{
    // uv = uv - (1/64, 1/64)
    // r = frac(uv.x * 32)
    // b = uv.x - frac(uv.x * 32) / 32
    // g = uv.y
    // 32 / 31
    uv -= params.yz;
    real3 color;

Shader "Hidden/Universal Render Pipeline/GaussianDepthOfField"
{
    HLSLINCLUDE
        ...
        half4 Blur(Varyings input, float2 dir, float premultiply)
        {
            ...
            float2 offset = _SourceSize.zw * _DownSampleScaleFactor.zw * dir * samp0CoC * MaxRadius;//与coc成正比的offset
            half4 acc = 0.0;
            UNITY_UNROLL
        ...
        half4 FragComposite(Varyings input) : SV_Target
        {
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
            float2 uv = UnityStereoTransformScreenSpaceTex(input.uv);
            half3 baseColor = LOAD_TEXTURE2D_X(_SourceTex, _SourceSize.xy * uv).xyz;//原颜色贴图
            ...
            if (coc > 0.0)
            {
                ...
                half blend = sqrt(coc * TWO_PI);//coc越大，blend越大
                dstColor = farColor * saturate(blend);
                dstAlpha = saturate(1.0 - blend);//coc越大，dstAlpha越小
            }
